(function () {
  ...
  angular.module('deliveryApp.customer').controller('RestaurantDetailController', RestaurantDetailController);

  RestaurantDetailController.$inject = ['PublicService', '$stateParams', 'CartService', 'UiFeedbackService', '$q'];
  function RestaurantDetailController(PublicService, $stateParams, CartService, UiFeedbackService, $q) {
    var vm = this;

    vm.restaurantId = Number($stateParams.restaurantId);
    vm.restaurant = null;
    vm.menu = [];
    vm.reviews = [];
    vm.loading = true;
    ...
    function load() {
      vm.loading = true;
      $q.all([
        PublicService.getRestaurant(vm.restaurantId),
        PublicService.getMenu(vm.restaurantId),
        PublicService.getReviews(vm.restaurantId, { page: 1, per_page: 5 }),
      ])
        ...
        .catch(function () {
          UiFeedbackService.error('Nao foi possivel carregar o restaurante.');
        })
        .finally(function () {
          vm.loading = false;
        });
